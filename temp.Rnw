...
\author{Gourp members: Yixuan Liu, Zhikang Liu, Yunke Wan, Katherine Cheng}
\date{\today}
...
\begin{figure}
<<echo=FALSE,message=FALSE,fig.width=8,fig.height=4,warning=FALSE>>=
# Create SQL queries to count delays
...
table_latex <- kable(head(initial, 15), format = "latex") # Print the LaTeX code for the table cat(table_latex)
table_latex
@
...
\frametitle{Departure delay time analysis}
\begin{figure}
\caption{Departure Delay Times in US States}
<<echo=FALSE,fig.width=8,fig.height=4,warning=FALSE>>=
...
% \begin{frame}
% \frametitle{Word Frequency: Airlines}
% \begin{center}
% <<fig.width=3, fig.height=3, echo=FALSE, warning=FALSE>>=
% airline_ratings <- read.csv("airline.csv")
% 
% delay_reviews <- airline_ratings[,c("airline_name", "date", "content", "overall_rating")] %>%
%   filter(str_detect(airline_ratings$content, "[Dd][Ee][Ll][Aa][Yy]"))
% 
% #install.packages("wordcloud")
% #install.packages("tm")
% text <- tolower(paste(delay_reviews$content, collapse = " "))
% corpus <- Corpus(VectorSource(text))
% corpus <- tm_map(corpus, content_transformer(tolower))
% corpus <- tm_map(corpus, removePunctuation)
% corpus <- tm_map(corpus, removeNumbers)
