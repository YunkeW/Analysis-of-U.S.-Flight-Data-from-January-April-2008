...
% Load required packages for figures, spacing, math, hyperlinks, etc.
\usepackage{graphicx}
\usepackage{setspace}
\usepackage{amsmath}
\usepackage{hyperref}
...
% Set the title and date for the presentation
...
%======================================================================
% First code chunk: Set up libraries and connection to the SQLite database
%======================================================================
<<echo=FALSE, message=FALSE, fig.width=8, fig.height=4, warning=FALSE>>=
# Define a function to load required libraries
...
# Set the working directory (change this path as needed on each computer)
setwd("C:/Users/78641/Downloads")

# Establish a connection to the SQLite database "final_project.db"
con <- dbConnect(RSQLite::SQLite(), dbname="final_project.db")
# Uncomment the next lines to inspect the database tables and fields:
# dbListTables(con)
# dbListFields(con, "main_data") 
@

%======================================================================
% Second code chunk: Query and clean the flight data
%======================================================================
<<echo=FALSE, message=FALSE, fig.width=8, fig.height=4, warning=FALSE>>=
library(dplyr)
library(ggplot2)
...
library(nortest)
library(MASS)
## IMPORTANT: Make sure you set the working directory correctly!

# Define an SQL query to retrieve selected columns from "main_data"
...
%======================================================================
% Frame: Category
% This slide lists the parts of the analysis
%======================================================================
...
%======================================================================
% Frame: Introduction slide with problem overview
%======================================================================
\begin{frame}
\frametitle{Introduction}
Problem Overview:\\
(1) Which one of our variables is the most significant factor for total delay?\\
(2) From different perspectives, is there any information about the distribution of American airports or airlines’ flight delays useful to travelers? \\
(3) How do delays impact customer satisfaction across American companies as well as international airports and airlines?\\
\end{frame}

%======================================================================
% Frame: Description of Dataset
% This slide presents a table with the dataset’s variables and their descriptions.
%======================================================================
\begin{frame}{Description of Dataset}
\begin{table}
  \centering
  \scalebox{0.6}{ % Adjust the scale as needed
  \begin{tabular}{|l|r|}
  \hline
  Variable Name & Description\\
  \hline
  Year  & Year of flight in YYYY \\
  Month  & Month of flight as numeric 1-12 \\
  DayofMonth  & Day of flight as numeric 1-31 \\
  DayofWeek  & Weekday of flight as numeric 1 (Monday) - 7 (Sunday) \\
  ...
  Cancelled  & if flight was cancelled, 1, or not, 0 \\
  Diverted  & 1 = yes, 0 = no \\
  CarrierDelay, WeatherDelay, NASDelay, SecurityDelay  & in minutes \\
  LateAircraftDelay  & in minutes \\
  \hline
  \end{tabular}
  }
  \caption{Variables Included in the Dataset, with Descriptions}\label{tab:simple-table}
\end{table}
\end{frame}

%======================================================================
% Frame: 2a – Lollipop Chart for Delay Categories
% This chunk queries the database for counts of different delay types and creates a lollipop chart.
%======================================================================
\begin{frame}{2a}
\frametitle{Categories of Delay}
\begin{figure}
<<echo=FALSE, message=FALSE, fig.width=8, fig.height=4, warning=FALSE>>=
# Create SQL queries to count the number of occurrences for each delay type.
...
# Execute the queries and convert the counts to numeric values.
...
# Combine the counts into a vector in a specific order.
c <- c(count_Carrier[2], count_LateAircraft[2], count_NAS[2], count_Security[2], count_Weather[2])

# Define the corresponding delay category names.
category <- c("Carrier", "LateAircraft", "NAS", "Security", "Weather")

# Create a data frame and arrange it by count.
data1 <- arrange(data.frame(category, c), c)

# Build a lollipop chart using ggplot2.
...
# Print the plot (suppress warnings)
suppressWarnings(print(lollipops))
@
\caption{Lollipop chart on the number of flight delays in five categories}
\end{figure}
\end{frame}

%======================================================================
% Frame: 2b – Density Plot for Departure Delay Analysis (Image Inclusion)
%======================================================================
\begin{frame}{2b}
\frametitle{Departure delay time analysis}
% Include a pre-generated image "density shiny.png" showing a density plot.
\includegraphics[width=\textwidth]{density shiny.png}
\end{frame}

%======================================================================
% Frame: 2b – Departure Delay Analysis with Hex Plot and Robust Regression
%======================================================================
\begin{frame}{2b}
\frametitle{Departure delay time analysis}
\begin{figure}
<<echo=FALSE, fig.width=8, fig.height=4, warning=FALSE>>=
# Load additional libraries and reconnect to the database
library(ggplot2)
con <- dbConnect(RSQLite::SQLite(), dbname="final_project.db")

# Run an SQL query to compute average departure delay, total flights, and percentage of delayed flights by Origin.
...
# Create a hex plot with robust linear regression (rlm) smoothing.
...
@
\caption{Percentage of Outbound Flights Delayed On Average Delay}
\end{figure}
\end{frame}

%======================================================================
% Frame: 2b – On-Time Performance Table for Airlines
%======================================================================
\begin{frame}{2b}
\frametitle{Departure delay time analysis}
\begin{table}
<<echo=FALSE, message=FALSE, fig.width=8, fig.height=4, warning=FALSE>>=
# Load libraries for table generation
library(knitr)
library(kableExtra)

# Get the unique levels of the 'airline' factor.
level_3 = levels(factor(result$airline))

# Define a function to split data by airline.
...
initial = data.frame()
performance = 0
# For each airline, calculate mean departure delay, variance, and on-time performance.
for (i in 1:length(level_3)){
    ...
    # Calculate on-time performance as the fraction of flights within one standard deviation.
...
library(xtable)
# Arrange and format the results table.
...
table_latex
@
\caption{Rated American air company (on-time performance)}
\end{table}
\end{frame}

%======================================================================
% Frame: 2c – Airline Delay Analysis (Image Inclusion)
%======================================================================
\begin{frame}{2c}
\frametitle{Departure delay time analysis}
...
%======================================================================
% Frame: 2c – U.S. States Map of Departure Delay Times
%======================================================================
\begin{frame}{2c}
\frametitle{Departure delay time analysis}
\begin{figure}
<<echo=FALSE, fig.width=8, fig.height=4, warning=FALSE>>=
# Query average departure delay by state by joining flight data with airport codes.
...
# Load libraries for mapping and color scales.
suppressMessages(library(usmap))
suppressMessages(library(ggplot2))
suppressMessages(library(viridis))
colors <- viridis(3)

% Create a U.S. map plot with average delay times.
...
%======================================================================
% Frame: Section 3 – International Comparisons by Text Analysis
%======================================================================
\begin{frame}
\frametitle{Section 3: International Comparisons by Text Analysis}
Using 2007-2015 customer reviews scraped from SkyTrax...
\begin{itemize}
  \item What words appear most frequently for reviews mentioning delays? 
  \item How do American airline companies and airports compare internationally?
\end{itemize}
\end{frame}

%======================================================================
% Frame: Word Frequency Analysis for Airports
%======================================================================
\begin{frame}
First, examined word frequency on reviews that mention delays.
\begin{itemize}
  \item Methodology 
  \item Results:
\end{itemize}
...
%======================================================================
% Frame: Word Cloud for Airport Reviews (Text Analysis)
%======================================================================
\begin{frame}
\frametitle{Word Frequency: Airports}
\begin{center}
<<fig.width=3, fig.height=3, echo=FALSE, warning=FALSE>>=
# Load libraries required for text mining and word cloud generation.
run_lib <- function(){
  library(RSQLite)
  library(ggplot2)
  library(dplyr)
...
% Query airport reviews from the database.
res <- dbSendQuery(conn = con, "
...
% Filter reviews that mention the word "delay" (case insensitive)
delay_reviews <- reviews %>%
  filter(str_detect(reviews$Review, "[Dd][Ee][Ll][Aa][Yy]"))

% Combine and preprocess the text for the word cloud.
...
%======================================================================
% Frame: Sentiment Analysis for Airports (Best Results)
%======================================================================
\begin{frame}
...
1 & singapore-changi-airport & 1255.00 \\ 
  ...
  20 & doha-airport & 258.00 \\ 
   \hline
\end{tabular}
\caption{Sample of best sentiment analysis results by airport (Red = American)} 
\end{table}
\end{frame}

%======================================================================
% Frame: Sentiment Analysis for Airports (Worst Results)
%======================================================================
\begin{frame}
\frametitle{Sentiment Analysis: Airports}
\begin{table}[H]
...
1 & london-stansted-airport & -906.00 \\ 
  ...
  20 & bournemouth-airport & -105.00 \\ 
   \hline
\end{tabular}
\caption{Sample of worst sentiment analysis results by airport} 
\end{table}
\end{frame}

%======================================================================
% Frame: Sentiment Analysis for Airlines (Best Results)
%======================================================================
\begin{frame}
...
1 & british-airways & 5258.00 \\ 
  ...
  20 & aer-lingus & 2154.00 \\ 
   \hline
\end{tabular}
\caption{Sample of best sentiment analysis results by airline} 
\end{table}
\end{frame}

%======================================================================
% Frame: Sentiment Analysis for Airlines (Worst Results)
%======================================================================
\begin{frame}
\frametitle{Sentiment Analysis: Airlines}
\begin{table}[ht]
...
1 & \textcolor{red}{spirit-airlines} & -976.00 \\ 
  ...
  20 & mokulele-airlines & -37.00 \\ 
   \hline
\end{tabular}
\caption{Sample of worst sentiment analysis results by airline} 
\end{table}
\end{frame}

% Disconnect the database connection at the end of the presentation.
